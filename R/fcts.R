...
#' @return n x (K+1) - matrix representing the value of each basis function 
...
#' @param fix_pars which parameters should be fixed: 'drift', 'boundary' or leave blank (default) for full model
...
LDDMM <- function(data, hypers, fix_pars = NULL, 
  ...
  else if (fix_pars == 'drift'){
    fit <- LDDMM_fix_drift(data, hypers, Niter, burnin, thin)
  }
  else if (fix_pars == 'boundary'){
    fit <- LDDMM_fix_bound(data, hypers, Niter, burnin, thin)
  }
  else {
    stop('fix_pars can be either drift or boundary!')
  }
...
LDDMM_fix_drift <- function(data, hypers, Niter = 5000, burnin = 2000, thin = 5){
